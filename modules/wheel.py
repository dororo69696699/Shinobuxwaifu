...
from database.models import get_user, get_collection, add_balance
...
def roll_wheel():
    sectors_list = [s for s in SECTORS]
...
async def spin_wheel(message: Message) -> None:
    ...
    if user_id in active_spins:
        await message.reply(
            "🎡 <b>Wheel</b>\n\n<blockquote>⏳ Your previous spin is processing!</blockquote>",
    ...
    try:
        ...
        if amount < 100 or amount > 50000:
            await message.reply("❌ Bet must be between 100 and 50,000!", parse_mode=ParseMode.HTML)
            return
    except ValueError:
    ...
    try:
        ...
        if winnings > amount:
            ...
            status_text = "⚖️ Push! Bet refunded."
        elif winnings > 0:
