...
async def shop_menu(message: Message) -> None:
    ...
    all_premium = await characters_collection.find({"rarity": PREMIUM_RARITY}).to_list(length=None)
    
    if not all_premium:
        await message.reply("🌸 No premium spirits available!\n\nPlease check back later.", parse_mode=ParseMode.HTML)
        return
    
    char_ids = [char['id'] for char in all_premium]
    stock_entries = await shop_stock_collection.find({'character_id': {'$in': char_ids}, 'stock': {'$gt': 0}}).to_list(length=None)
    ...
    if not characters:
        await message.reply("🌸 No premium spirits in stock!\n\nPlease check back later.", parse_mode=ParseMode.HTML)
        return
...
async def display_shop_character(message, user_id, is_initial=False, callback_query=None):
    ...
    index, characters = state["index"], state["characters"]
    stock_dict, price_dict = state["stock_dict"], state["price_dict"]
    ...
    if is_initial:
        await message.reply_photo(photo=img_url, caption=caption, reply_markup=reply_markup, parse_mode=ParseMode.HTML)
    else:
        await callback_query.message.edit_caption(caption=caption, reply_markup=reply_markup, parse_mode=ParseMode.HTML)
        await callback_query.answer()


@router.callback_query(lambda c: c.data and c.data.startswith("shop_next_"))
async def shop_next(callback: CallbackQuery) -> None:
    user_id = int(callback.data.split("_")[2])
    if callback.from_user.id != user_id or user_id not in user_shop_state:
        await callback.answer("Session expired or not yours!", show_alert=True)
        return
    user_shop_state[user_id]["index"] += 1
    await display_shop_character(callback.message, user_id, callback_query=callback)


@router.callback_query(lambda c: c.data and c.data.startswith("shop_prev_"))
...
async def shop_buy(callback: CallbackQuery) -> None:
    ...
    user_id, index = int(data[2]), int(data[3])
    ...
    if new_stock == 0:
        ...
        await shop_stock_collection.update_one({'character_id': char_id}, {'$set': {'stock': new_stock}})
    
    users_collection = get_collection("users")
    await users_collection.update_one(
        {"id": user_id},
        {"$inc": {"balance": -discounted_price}, "$push": {"characters": char}}
    )
